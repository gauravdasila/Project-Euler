...
function greatestProductInGrid() {
    var greatestProd = 1, product;
    for(var i = 0 ; i < grid.length ; i++ ) {
        for(var j = 0 ; j < grid[i].length ; j++ ) {
            ...
            if(product > greatestProd) greatestProd = product;
            /* top side */
            product = 1;
              for(var k = i ; k > i - 4 ; k-- ) {
                if( k > 0) {
            ...
            /* diagonal to right */
            product = 1;
            for(var x = i , y = j; x < i + 4 , y < j + 4 ; x++ , y++ ) {
